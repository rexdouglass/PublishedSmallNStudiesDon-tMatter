...
- Local observable row universe: **3,032 rows**.
- Strict current Figure 1 included rows: **1,684** (55.5%).
- Root rows with D/N values whether included or blocked: **2,436** (80.3%).
- D/N rows blocked by the current plot rule: **752** (24.8%).
...
- Roster-only linked pairs without public effect/N values: **248** (8.2%).
- Effect-value rows with no public N columns: **172** (5.7%).
