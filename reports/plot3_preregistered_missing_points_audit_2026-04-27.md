...
| Brodeur economics preregistered/PAP table-test rows | 10,071 extracted table-test rows | 3,724 preregistration/PAP/PAP-power flagged D/N rows across 70 papers | Real local source of the remembered "thousands": row notes contain `prereg`, `preanalysisplan`, and `pap_power` flags. Excluded under the current gate because all rows are extracted table tests without a focal/main-result selector. |
...
The biggest optional future change is not extraction; it is a scope decision. If Plot 3 is broadened from "published analytic preregistered confirmatory results" to "all preregistered or registered outcomes," then the first new sensitivity lane should be ClinicalTrials.gov registry results (`+7,722` rows), the second should be Brodeur preregistered/PAP economics table tests (`+3,724` extracted test rows, or 70 paper-collapsed rows), and the third should be CliniFact published registry-linked primary outcomes (`+339` rows).
