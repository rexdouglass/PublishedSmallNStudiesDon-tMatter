    ...
    plot3 = pd.DataFrame(
        [
            ...
            },
            {
                ...
                "what_it_is_why_possible_candidate": "The SCORE/COS claim corpus was rechecked because its public OSF files expose separate original-study effect sizes, effective-N fields, and `orig_prereg-indicated.csv`. The local SCORE D/N parser already reconstructs paper-level D/N for a subset of preregistration-indicated papers.",
                "confirmed_fields": f"Known: {fmt_int(score_prereg_count)} unique SCORE paper IDs marked `prereg=True`, {fmt_int(len(score_prereg))} deduplicated paper-level D/N rows. Confirmed: paper-level preregistration indication: yes ({fmt_int(score_prereg_count)}); public flat payloads: `orig_effect-size.csv`, `orig_stats_effective.csv`, and `orig_prereg-indicated.csv`; paper grouping: yes; D/N: yes ({fmt_int(len(score_prereg))}); claim-specific preregistration mapping: no; support status: not coded.",
