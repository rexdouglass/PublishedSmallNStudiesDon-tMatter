...
Figure 3 is restricted to preregistered confirmatory result rows or paper/project medians of such rows: the underlying statistic must map to a precommitted hypothesis/result and have recoverable `D` and `N`. The comparator panels below deliberately show large local sources that are close enough to inspect but not comparable enough to enter Figure 3.
...
- the unit is a preregistered hypothesis/result row or a median collapse of multiple matched planned-result rows from the same paper/project,
