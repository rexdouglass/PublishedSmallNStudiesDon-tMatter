...
<nav class="dataset-contents" aria-label="Contents">
  ...
  <ul>
    ...
    <li><a href="#provenance-schema-and-100-row-retrace-pilot">Provenance Schema</a></li>
...
{{< include _generated/provenance_schema_pilot.qmd >}}
...
{{< include _generated/figure1_replication_review.qmd >}}
...
Plot 3 is the preregistered-results comparison layer, not a registry layer. Under the relaxed rule, "confirmatory" means the result is inside a plan written before the result was known. A paper no longer needs a single main-result selector if the local source identifies multiple preregistered hypothesis results; those planned results are collapsed to one median dot per paper/project before plotting when the source exposes a matched result set.
...
Figure 3 is restricted to one dot per paper/project: either a single preregistered confirmatory result or a median of multiple matched planned-result rows. The underlying statistic must map to a precommitted hypothesis/result and have recoverable `D` and `N`. The comparator panels below deliberately show large local sources that are close enough to inspect but not comparable enough to enter Figure 3.
...
The larger ClinicalTrials.gov API pass below is the scale check. It uses the official API record, not the older KG proxy, and keeps registered primary, secondary, and other pre-specified outcome-analysis rows when a two-group denominator is present and `D` is recoverable from a ratio estimate, event-count/proportion row, or p-value proxy. The row-level file currently has `190,836` registered outcome-analysis rows from `17,726` randomized interventional trials; the figure plots one median per trial so the visual is not inflated by trials with hundreds of reported outcomes.

![ClinicalTrials.gov API registered outcome-result trial medians, one median row per randomized interventional trial.](_generated/figures/plot3_ctgov_api_registered_trial_medians.png)

## Inclusion Criteria

Plot 3 should include a source family when:

- the row family represents **planned preregistered results** rather than post hoc exploratory findings,
- `D` and `N` are recoverable or already encoded at the result level,
- the source is public and locally auditable,
- the plotted unit is one paper/project dot: either a preregistered hypothesis/result row or a median collapse of multiple matched planned-result rows from the same paper/project,
- and the source does not merely say "this paper was preregistered" while leaving the plotted statistic unlinked to the preregistered plan.
...
- ClinicalTrials.gov API registered outcome-analysis rows: [plot3_ctgov_api_registered_outcome_ratio_event_rows.csv.gz](../data/derived/effect_inflation_dataset/plot3_ctgov_api_registered_outcome_ratio_event_rows.csv.gz)
- ClinicalTrials.gov API one-trial median rows: [plot3_ctgov_api_registered_trial_medians.csv](../data/derived/effect_inflation_dataset/plot3_ctgov_api_registered_trial_medians.csv)
- RPCB preclinical Registered Report medians: [plot3_rpcb_preclinical_paper_level_candidates.csv](../data/derived/effect_inflation_dataset/plot3_rpcb_preclinical_paper_level_candidates.csv)
...
- the SCORE and van den Akker paper-median expansions,
- the RPCB preclinical Registered Report medians,
