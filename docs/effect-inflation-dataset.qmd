...
The larger ClinicalTrials.gov API pass below is the scale check. It uses the official API record, not the older KG proxy, and keeps registered primary, secondary, and other pre-specified outcome-analysis rows when a two-group denominator is present and `D` is recoverable from a ratio estimate, event-count/proportion row, or p-value proxy. The row-level file currently has `67,486` registered outcome-analysis rows from `5,628` randomized interventional trials; the figure plots one median per trial so the visual is not inflated by trials with hundreds of reported outcomes.
...
- ClinicalTrials.gov API registered outcome-analysis rows: [plot3_ctgov_api_registered_outcome_ratio_event_rows.csv](../data/derived/effect_inflation_dataset/plot3_ctgov_api_registered_outcome_ratio_event_rows.csv)
