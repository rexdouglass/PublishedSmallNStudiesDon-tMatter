...
The strict Plot 3 panel excludes registry-only and table-test-only evidence. The sidecar below shows the two large local preregistered-like D/N sources that triggered the “thousands” question: ClinicalTrials.gov registry-result rows and Brodeur preregistered/pre-analysis-plan economics table tests. This is a sensitivity panel, not an expansion of the strict confirmatory-result corpus.
...
The cleanest ClinicalTrials.gov subset we can build from the local files is stricter but still not strict-Plot-3 evidence: phase-2+ randomized interventional trials with exactly one locally eligible primary two-group outcome row, plotted one row per trial. It remains a sidecar because `D` is still a registry p-value/enrollment proxy and the row has not been matched to a locked protocol or SAP analysis.
